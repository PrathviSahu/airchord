...
export class GestureEngine {
  ...
  countFingers(landmarks: { x: number; y: number; z: number }[]): number {
    ...
    // Simple approach: thumb is extended if tip is clearly away from the palm
    ...
    const indexExtended = (indexPIP.y - indexTip.y) > 0.04;

    // Middle: tip (12) significantly above PIP (10)
    const middleTip = landmarks[12];
    const middlePIP = landmarks[10];
    const middleExtended = (middlePIP.y - middleTip.y) > 0.04;

    // Ring: tip (16) significantly above PIP (14)
    const ringTip = landmarks[16];
    const ringPIP = landmarks[14];
    const ringExtended = (ringPIP.y - ringTip.y) > 0.04;
