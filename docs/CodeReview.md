...
- smooth audio muting and an immediate beat-one start.
...
3. **The website bundle is large.** The production build is about 1.55 MB minified before gzip, and `website/public/models/guitar.glb` is about 59 MB. Lazy-load the Three.js landing scene, split the studio route, Draco/Meshopt-compress the model, and set a performance budget in CI.
...
5. **Gesture confidence is hard-coded.** `GestureEngine` reports `0.95` for every accepted hand and does not use MediaPipe confidence/handedness to reject low-quality frames. Pass the detector score through `HandResult`, calibrate thresholds with recorded landmarks, and expose “uncertain” rather than forcing a chord.
6. **Frame processing is still main-thread work.** `detectForVideo` runs from a `requestAnimationFrame` loop. Use a worker where supported, throttle to a target rate (for example 24–30 FPS), and keep audio scheduling independent from camera frame load.
7. **Live pause/resume is not a true transport.** The lyric clock restarts from zero on resume and the interval resets its beat index. Store a beat/transport position and schedule against `AudioContext.currentTime` so pause, resume, and background-tab throttling stay musical.
