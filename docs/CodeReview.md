...
- context-safe physical-model/sample caches and disposed MediaPipe resources;
- recorder URL/timer cleanup, late-camera-stream cancellation, and unsupported custom-chord validation;
- timestamp-based LRC-to-chord matching and correct song-pattern handoff into practice mode.
...
| P2 | Website had no explicit typecheck or data-validation test command. | Type errors and broken chord/song references were easy to miss because Vite transpiles without typechecking. | Added `npm run typecheck`, `npm run test`, six Vitest checks, and aligned website React type packages with React 18. |
...
| P1 | A late `getUserMedia()` resolution could attach a camera stream after the route had unmounted. | Camera tracks and permission resources could leak during fast navigation. | Added cancellation guards and late-stream track cleanup in live/practice screens. |
| P1 | MediaRecorder output URLs, timers, and active recorders were not fully disposed on route changes. | Long sessions could leak Blob URLs or keep recording resources alive. | Added URL revocation, mounted guards, recorder/timer cleanup, and error handlers. |
| P1 | External LRC lines were matched to chords by array index. | Different lyric segmentation assigned chords to the wrong lyric lines. | Match local chord events by timestamp and memoize the merged timeline. |
...
- Expand the six unit tests with Web Audio mocks and a browser smoke test for mocked camera/model/network dependencies.
